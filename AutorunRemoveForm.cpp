...
// ---------------------------------------------------------------------------
__fastcall TAutorunRemove::TAutorunRemove(TComponent* Owner) : TForm(Owner) {
}
// ---------------------------------------------------------------------------
...
void __fastcall TAutorunRemove::btn2Click(TObject *Sender) {
	if (lv1->SelCount > 0) {
		...
		if (::MessageBox(this->Handle, _T("你确定要删除该项吗?"), _T("确认"),
		...
		::MessageBox(this->Handle, _T("请首先选择需要删除的项！"), _T("警告"),
		MB_ICONWARNING);
	}
}
// ---------------------------------------------------------------------------
